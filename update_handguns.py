...
async def fetch_duckduckgo_image(playwright, search_query):
    browser = await playwright.chromium.launch(headless=True)
    page = await browser.new_page()
    page.set_default_timeout(10000)  # Set overall timeout to 10s
    query = quote(search_query)
    search_url = f"https://duckduckgo.com/?q={query}&iax=images&ia=images"
    try:
        await page.goto(search_url)
        await page.wait_for_selector("img.tile--img__img", timeout=5000)
    ...
    async with async_playwright() as p:
        ...
        for row in rows[1:]:
            ...
            if len(cells) >= 5:
                ...
                image_url = await fetch_duckduckgo_image(p, search_query)
