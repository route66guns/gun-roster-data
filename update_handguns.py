...
async def fetch_gunbroker_image(playwright, search_query):
    browser = await playwright.chromium.launch(headless=True)
    page = await browser.new_page()
    search_url = f"https://www.gunbroker.com/All/search?Keywords={search_query}"
    try:
        await page.goto(search_url, timeout=60000)
        await page.wait_for_selector(".item-card img", timeout=10000)
        img_el = await page.query_selector(".item-card img")
        if img_el:
            img_url = await img_el.get_attribute("src")
            return img_url if img_url else PLACEHOLDER_IMAGE
    except Exception as e:
        print(f"Image fetch failed for {search_query}: {e}")
    finally:
        await browser.close()
    ...
    async with async_playwright() as p:
        ...
        for row in rows[1:]:
            ...
            if len(cells) >= 5:
                ...
                search_query = f"{manufacturer} {model}".replace(" ", "+")
                print(f"🔍 Searching GunBroker for: {search_query}")
                image_url = await fetch_gunbroker_image(p, search_query)
                time.sleep(2)  # gentle delay to avoid blocking
