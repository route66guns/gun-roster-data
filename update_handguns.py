...
async def fetch_duckduckgo_image_api(search_query, fallback_query=None):
    def shorten(query):
        return query if len(query) <= MAX_QUERY_LENGTH else query.split(" ")[0] + " handgun"

    search_query = shorten(search_query)
        ...
        async with aiohttp.ClientSession(headers=headers) as session:
            ...
            async with session.get(api_url) as api_resp:
                ...
                try:
                    ...
                    if fallback_query:
                        return await fetch_duckduckgo_image_api(fallback_query)
    ...
    async with async_playwright() as p:
        ...
        for row in rows[1:]:
            ...
            if len(cells) >= 5:
                ...
                fallback_query = f"{cleaned_manufacturer} handgun"
                print(f"🔍 API image search: {search_query}")
                image_url = await fetch_duckduckgo_image_api(search_query, fallback_query)
                time.sleep(1)
